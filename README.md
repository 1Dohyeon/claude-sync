...
응답 규칙과 작업 규칙이 적혀 있어, 어느 세션에서든지 공통된 작업 스타일을 갖는다.

- 주제와 무관하게(코드·연구·조사·문서 등) **설계 → 보고 → 진행 → 검증 → 완료** 순으로 진행한다. 단순 조회·질문·한 줄 수정에는 적용하지 않는다.
- 개인적인 작업 규칙을 두 곳에 나눠 담는다. **항상 적용되는 규율은 `rules/`**, **특정 상황에서만 필요한 절차는 `skills/`** 이다. `rules/`는 세션 시작 시 자동으로 로드되고, `skills/`는 `CLAUDE.md`의 ROUTING CONVENTION 표에 따라 호출된다.
