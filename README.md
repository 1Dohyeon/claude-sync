...
6. **리뷰**: 범위에 따라 스킬이 갈립니다. 커밋 전이면 [`/diff-review`](skills/diff-review/SKILL.md), PR을 올리기 전 브랜치 전체면 [`/branch-review`](skills/branch-review/SKILL.md), 올라온 PR이면 [`/pr-review`](skills/pr-review/SKILL.md)입니다. 스킬 이름이 곧 범위 선언이라 대상을 되묻는 단계가 없습니다.
   ...
   - 6-2. **병렬 리뷰**: 각 축이 자기 영역만 보도록 격리된 서브에이전트가 병렬로 봅니다. 축마다 모델을 따로 지정합니다. 호출부를 열어 실행 경로를 따라가야 하는 코드 레벨 축은 Opus로 두고, 문서·설정·인접 코드와 대조하면 끝나는 나머지 축은 Sonnet으로 내립니다.
   - 6-2-1. **코드 레벨 축 분할**: 전체 소요 시간은 가장 느린 축 하나가 결정합니다. 코드 레벨 축이 다른 축보다 3분가량 오래 걸리므로, 변경이 600줄 또는 파일 15개를 넘으면 파일을 두 묶음으로 갈라 이 축만 두 개를 동시에 띄웁니다. 그보다 작으면 나누지 않습니다. 나누는 비용이 얻는 시간보다 커지기 때문입니다.
   - 6-3. **취합**: 축이 하나씩 도착하는 동안 중간 보고를 하지 않고 전부 모일 때까지 기다립니다. 도착할 때마다 한 턴씩 쓰면 그만큼 끝이 밀립니다. 결과가 다 오면 상위 모델이 중복을 합치고 등급 순으로 하나의 리포트에 정리합니다. 등급은 에이전트가 붙이지 않고, 각 축이 낸 사실을 상위가 하나의 기준으로 환산합니다. 이 단계에서 새 지적은 만들지 않고, 상반된 결론은 억지로 해소하지 않고 나란히 둡니다. 코드 레벨 축을 둘로 나눠 띄웠으면 같은 지적이 양쪽에서 나올 수 있으므로, 중복 병합을 이 두 결과 사이에서도 적용합니다.
...
```mermaid
flowchart LR
    D["/diff-review<br/>워킹트리 변경"] --> LG["logic-reviewer<br/>Opus"]
    D --> CV["convention-reviewer<br/>Sonnet"]
    D -.-> LG2["logic-reviewer 2호<br/>Opus · 600줄 또는 15파일을 넘을 때만"]
...
`/branch-review`와 `/pr-review`는 보는 범위만 다르고 축 구성이 같아서 한 노드로 묶었습니다. 점선으로 들어가는 화살표는 조건이 맞을 때만 호출된다는 뜻입니다. `qa-reviewer`에서 나가는 화살표도 점선인데, 이 결과는 지적이 아니라서 검증과 등급을 거치지 않고 리포트 맨 뒤에 그대로 붙기 때문입니다.

노드에 붙은 모델 이름은 그 축을 어느 모델로 띄우는지를 뜻하며, [`agents/`](agents/)의 각 정의 앞머리에 적혀 있습니다. 코드 레벨 축만 Opus인 것은 이 축이 diff만 읽고 끝나지 않고 호출부를 따라가며 어떤 값이 들어올 수 있는지 확인해야 하기 때문입니다. `logic-reviewer 2호`는 별도의 에이전트 정의가 아니라 같은 정의를 파일 묶음만 달리 해서 한 번 더 띄우는 것입니다.

### 왜 이렇게 나눴는가

과거 세션 기록에서 축별 소요 시간을 재어 정한 배치입니다. 병렬로 띄워도 전체가 끝나는 시점은 가장 느린 축이 정하므로, 편차를 줄이는 것이 축을 더 늘리는 것보다 효과가 큽니다.

| 축 | 중앙값 | 조치 |
| --- | --- | --- |
| logic-reviewer | 449초 | Opus 유지, 규모가 크면 2분할 |
...
코드 레벨 축이 끝날 때 나머지 축은 이미 3분 가까이 결과를 내놓고 기다리는 상태였습니다. 그래서 다른 축을 더 빠르게 만드는 것은 전체 시간에 거의 듣지 않고, 토큰 비용에만 듣습니다.
