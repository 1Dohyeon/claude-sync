...
- 유사도가 낮은 결과(0에 가까움)까지 끌어오지 않는다. 눈에 띄게 높은 것만 열어 본다. 계획 문서는 결과에 나온 폴더의 `requirements.md`를, 없으면 `tasks.md`를 `Read`로 열고, PR은 `gh pr view <번호>`로, 커밋은 `git show --stat <해시>`로 연다. 열어 보니 무관하면 버리고, 관련 있으면 무엇을 했는지 한두 줄로 남긴다.
