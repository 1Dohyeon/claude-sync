...
- **작업 문서는 세션을 넘길 때만 만든다.** 한 세션에 끝나는 작업은 이어받을 사람이 없어서 진행 로그가 쓸모없다.
...
- 보고서는 `/task <대상 브랜치>`로 만든다. 템플릿을 그대로 복사하므로 골격을 손으로 짜지 않는다.
...
- 세션이 미완료로 끝나면 `tasks/<대상 브랜치>.work.md`를 만들고([`templates/task-work.md`](../templates/task-work.md)를 `cp`) `## 진행` 아래에 회차를 덧붙여 "한 것 / 막힌 것 / 다음 할 것"을 적는다.
...
완료 처리 전에 작업 문서의 `## 검증`에 무엇을 어떻게 확인했는지 남긴다. 한 세션에 끝나서 작업 문서가 없는 작업은 대화로 보고한다.

### 완료

사용자가 통과로 확인한 뒤에만 수행한다. 사용자 허락 없이 `done/`으로 옮기지 않는다.

- 옮기기 전에 작업 문서 끝의 `## 완료`를 채운다. 작업 문서가 없으면 보고서만 옮긴다.
